...
*/
    ...
    #endif
#else
sdio_irq_handler_t mt_wifi_irq_handler = NULL;
pm_message_t mt_wifi_pm_state = { .event = PM_EVENT_HIBERNATE };
pm_callback_t mt_wifi_pm_cb = NULL;
void *mt_wifi_pm_data = NULL;
void *mt_wifi_irq_data = NULL;
int mt_wifi_pm_late_cb = 0;
#endif
/*=======================================================================*/
...
#if !defined(CONFIG_MTK_WCN_CMB_SDIO_SLOT)
void mt_wifi_enable_irq(void)
{
    mt65xx_eint_unmask(CUST_EINT_WIFI_NUM);
}

void mt_wifi_disable_irq(void)
{
    mt65xx_eint_mask(CUST_EINT_WIFI_NUM);
}

void mt_wifi_eirq_handler(void)
{
...
void mt_wifi_request_irq(sdio_irq_handler_t irq_handler, void *data)
{
...
void mt_wifi_register_pm(pm_callback_t pm_cb, void *data)
{
    /* register pm change callback */
    mt_wifi_pm_cb = pm_cb;
...
int mt_wifi_suspend(pm_message_t state)
{
    ...
    if (is_1st_suspend_from_boot) {
        ...
        if (mtk_wcn_cmb_sdio_pm_cb) {
            ...
            MT65xx-SD: sd host is added to MMC stack and suspend is ZERO by default
            ...
            shall be called once to let MT65xx-SD do sd host suspend and remove
...
#define MSDC_EMMC_FLAG    (MSDC_SYS_SUSPEND | MSDC_HIGHSPEED | MSDC_UHS1 |MSDC_DDR)
    ...
    struct msdc_hw msdc0_hw = {
        ...
        .cmd_drv        = 2, //3mA
        .dat_drv        = 2, //3mA
        .clk_drv_sd_18  = 2, //3mA for HS200
        .cmd_drv_sd_18  = 2, //3mA for HS200
        .dat_drv_sd_18  = 2, //3mA for HS200
        .data_pins      = 8,
        .data_offset    = 0,
    ...
    struct msdc_hw msdc1_hw = {
        ...
        .rdata_edge     = MSDC_SMPL_FALLING,
        .wdata_edge     = MSDC_SMPL_FALLING,
        .clk_drv        = 2, //6mA for SD20, SDR25
        .cmd_drv        = 2, //6mA for SD20, SDR25
        .dat_drv        = 2, //6mA for SD20, SDR25
        .clk_drv_sd_18  = 4, //6mA for SDR50, DDR50, SDR104
        .cmd_drv_sd_18  = 4, //6mA for SDR50, DDR50, SDR104
        .dat_drv_sd_18  = 4, //6mA for SDR50, DDR50, SDR104
        .data_pins      = 4,
        .data_offset    = 0,
        .flags          = MSDC_SDCARD_FLAG,
        .dat0rddly      = 0,
