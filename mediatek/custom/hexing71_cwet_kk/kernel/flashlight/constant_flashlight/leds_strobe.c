...
Functions
*****************************************************************************/
#define CONSTANT_FLASHLIGHT_USED_ISINK
...
#if defined(CONSTANT_FLASHLIGHT_USED_ISINK)    
...
{
	PK_DBG("FL_dim_duty2 %d", duty);


#if defined(CONSTANT_FLASHLIGHT_USED_ISINK)       
    kal_uint32 tmp_duty = 0;
    if (duty>31)
        tmp_duty = 31;
    else
        tmp_duty = duty;
    printk("tek.xing duty = %d\n",tmp_duty);
    upmu_set_isink_dim0_duty(tmp_duty);
    upmu_set_isink_dim1_duty(tmp_duty);
    upmu_set_isink_dim2_duty(tmp_duty);
    upmu_set_isink_dim3_duty(tmp_duty);
#else
