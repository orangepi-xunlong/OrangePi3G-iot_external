...
#define hwPowerDown	My_hwPowerDown
#define hwPowerOn 	My_hwPowerOn

bool  My_hwPowerDown(MT65XX_POWER powerId, char *mode_name)
{ 
	pmic_ldo_enable(powerId, KAL_FALSE);
	return true ;
}

bool My_hwPowerOn(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt, char *mode_name)
{
	pmic_ldo_vol_sel(powerId, powerVolt);   		 
	pmic_ldo_enable(powerId, true ); 
	return true ;
}
...
if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_OFF])){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
...

        {
            ...
            if(TRUE != hwPowerOn(PMIC_APP_MAIN_CAMERA_POWER_IO, VOL_1800,"camera"))
            {
        ...
        //AVDD
        if(TRUE != hwPowerOn(PMIC_APP_MAIN_CAMERA_POWER_A, VOL_2800,"camera"))
        {
        ...
        //DVDD
	{
            if(TRUE != hwPowerOn(PMIC_APP_MAIN_CAMERA_POWER_D, VOL_1800,"camera"))
        {
             PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
             //return -EIO;
             //goto _kdCISModulePowerOn_exit_;
	...
	if((0 != strcmp(SENSOR_DRVNAME_OV5645_MIPI_YUV,currSensorName)))
	{
        //AF_VCC
        if(TRUE != hwPowerOn(PMIC_APP_MAIN_CAMERA_POWER_AF, VOL_2800,"camera"))
        {
            PK_DBG("[CAMERA SENSOR] Fail to enable analog power\n");
            //return -EIO;
            goto _kdCISModulePowerOn_exit_;
...
mdelay(5);
     ...
     mdelay(10);
        ...
        if(TRUE != hwPowerDown(PMIC_APP_MAIN_CAMERA_POWER_AF,"camera"))
        {
            PK_DBG("[CAMERA SENSOR] Fail to enable analog power\n");
            //return -EIO;
            goto _kdCISModulePowerOn_exit_;
        }
        if(TRUE != hwPowerDown(PMIC_APP_MAIN_CAMERA_POWER_D, "camera")) {
            PK_DBG("[CAMERA SENSOR] Fail to OFF digital power\n");
            //return -EIO;
            goto _kdCISModulePowerOn_exit_;
        }
    	if(TRUE != hwPowerDown(PMIC_APP_MAIN_CAMERA_POWER_A,"camera")) {
            PK_DBG("[CAMERA SENSOR] Fail to OFF analog power\n");
            //return -EIO;
            goto _kdCISModulePowerOn_exit_;
        }
        if(TRUE != hwPowerDown(PMIC_APP_MAIN_CAMERA_POWER_IO,"camera"))
        {
            PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
            //return -EIO;
            goto _kdCISModulePowerOn_exit_;
...
mt_set_gpio_mode(pinSet[0][IDX_PS_CMPDN],0);
mt_set_gpio_dir(pinSet[0][IDX_PS_CMPDN],GPIO_DIR_OUT);
mt_set_gpio_out(pinSet[0][IDX_PS_CMPDN],1);

mt_set_gpio_mode(pinSet[0][IDX_PS_CMRST],0);
mt_set_gpio_dir(pinSet[0][IDX_PS_CMRST],GPIO_DIR_OUT);
mt_set_gpio_out(pinSet[0][IDX_PS_CMRST], 1);

/*
mt_set_gpio_mode(pinSet[1][IDX_PS_CMRST],0);
mt_set_gpio_dir(pinSet[1][IDX_PS_CMRST],GPIO_DIR_OUT);
mt_set_gpio_out(pinSet[1][IDX_PS_CMRST],0);

mt_set_gpio_mode(pinSet[1][IDX_PS_CMPDN],0);
mt_set_gpio_dir(pinSet[1][IDX_PS_CMPDN],GPIO_DIR_OUT);
mt_set_gpio_out(pinSet[1][IDX_PS_CMPDN],0);
*/
...
int Af_VCOMA2_Power_On(void)
{
    ...
    if(TRUE != hwPowerOn(PMIC_APP_MAIN_CAMERA_POWER_AF,VOL_1800,"camera")) //
#else
    if(TRUE != hwPowerOn(PMIC_APP_MAIN_CAMERA_POWER_AF,VOL_2800,"camera")) //
#endif
    {
