...
#ifdef DEBUG_SENSOR_GC2155
#define gc2155_OP_CODE_INI		0x00		/* Initial value. */
#define gc2155_OP_CODE_REG		0x01		/* Register */
#define gc2155_OP_CODE_DLY		0x02		/* Delay */
#define gc2155_OP_CODE_END		0x03		/* End of initial setting. */
...
} gc2155_initial_set_struct;

gc2155_initial_set_struct gc2155_Init_Reg[5000];

static u32 strtol(const char *nptr, u8 base)
{

	printk("gc2155___%s____\n",__func__); 

	u8 ret;
	if(!nptr || (base!=16 && base!=10 && base!=8))
	{
		printk("gc2155 %s(): NULL pointer input\n", __FUNCTION__);
		return -1;
	}
	for(ret=0; *nptr; nptr++)
	{
...
static u8 GC2155_Initialize_from_T_Flash()
{
	...
	printk("gc2155___%s____11111111111111\n",__func__); 
	...
	fp = filp_open("/mnt/sdcard/gc2155_sd.txt", O_RDONLY , 0); 
	if (IS_ERR(fp)) 
	{ 
	...
	printk("gc2155___%s____22222222222222222\n",__func__); 
	...
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			gc2155_Init_Reg[i].op_code = gc2155_OP_CODE_REG;

			gc2155_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */

			gc2155_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		...
		else									/* DLY */
		{
			...
			curr_ptr += 4;	
			gc2155_Init_Reg[i].op_code = gc2155_OP_CODE_DLY;

			gc2155_Init_Reg[i].init_reg = 0xFF;
			gc2155_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
	...
	gc2155_Init_Reg[i].op_code = gc2155_OP_CODE_END;
	gc2155_Init_Reg[i].init_reg = 0xFF;
	gc2155_Init_Reg[i].init_val = 0xFF;
	i++;
	//for (j=0; j<i; j++)
	printk("gc2155 %x  ==  %x\n",gc2155_Init_Reg[j].init_reg, gc2155_Init_Reg[j].init_val);
	
	printk("gc2155___%s____3333333333333333\n",__func__); 

	/* Start apply the initial setting to sensor. */
#if 1
	for (j=0; j<i; j++)
	{
		if (gc2155_Init_Reg[j].op_code == gc2155_OP_CODE_END)	/* End of the setting. */
		{
			printk("gc2155 REG OK -----------------END!\n");
		
			break ;
		}
		else if (gc2155_Init_Reg[j].op_code == gc2155_OP_CODE_DLY)
		{
			msleep(gc2155_Init_Reg[j].init_val);		/* Delay */
			printk("gc2155 REG OK -----------------DLY!\n");			
		}
		else if (gc2155_Init_Reg[j].op_code == gc2155_OP_CODE_REG)
		{

			GC2155_write_cmos_sensor(gc2155_Init_Reg[j].init_reg, gc2155_Init_Reg[j].init_val);
			printk("gc2155 REG OK!-----------------REG(0x%x,0x%x)\n",gc2155_Init_Reg[j].init_reg, gc2155_Init_Reg[j].init_val);			
			printk("gc2155 REG OK!-----------------REG(0x%x,0x%x)\n",gc2155_Init_Reg[j].init_reg, gc2155_Init_Reg[j].init_val);			
			printk("gc2155 REG OK!-----------------REG(0x%x,0x%x)\n",gc2155_Init_Reg[j].init_reg, gc2155_Init_Reg[j].init_val);			
...
static void GC2155_Sensor_Init(void)
{
	...
	GC2155_write_cmos_sensor(0x24 , 0x16);
	...
	GC2155_write_cmos_sensor(0x85 , 0x08); // 08 20140722
	...
	GC2155_write_cmos_sensor(0x43 , 0x5b);
	...
	GC2155_write_cmos_sensor(0x12 , 0x35);
	...
	GC2155_write_cmos_sensor(0x97 , 0x64);
	GC2155_write_cmos_sensor(0x98 , 0x88);
	GC2155_write_cmos_sensor(0x9d , 0x08);
	...
	GC2155_write_cmos_sensor(0x83 , 0x04);
	...
	GC2155_write_cmos_sensor(0xab , 0x40);
	...
	///////////////////gamma1////////////////////
	GC2155_write_cmos_sensor(0xfe , 0x02);
	GC2155_write_cmos_sensor(0x10 , 0x0d);
	GC2155_write_cmos_sensor(0x11 , 0x12);
	GC2155_write_cmos_sensor(0x12 , 0x17);
	...
	GC2155_write_cmos_sensor(0x1b , 0x9c);
	GC2155_write_cmos_sensor(0x1c , 0xaa);
	GC2155_write_cmos_sensor(0x1d , 0xbb);
	GC2155_write_cmos_sensor(0x1e , 0xca);
	GC2155_write_cmos_sensor(0x1f , 0xd5);
	...
	///////////////////gamma2////////////////////
	GC2155_write_cmos_sensor(0xfe , 0x02);
	GC2155_write_cmos_sensor(0x26 , 0x0d);
	GC2155_write_cmos_sensor(0x27 , 0x12);
	GC2155_write_cmos_sensor(0x28 , 0x17);
	...
	GC2155_write_cmos_sensor(0x31 , 0x9c);
	GC2155_write_cmos_sensor(0x32 , 0xaa);
	GC2155_write_cmos_sensor(0x33 , 0xbb);
	GC2155_write_cmos_sensor(0x34 , 0xca);
	GC2155_write_cmos_sensor(0x35 , 0xd5);
	...
	/////////////////////////////////////////////// 
	GC2155_write_cmos_sensor(0xfe , 0x02);
	GC2155_write_cmos_sensor(0xd1 , 0x28);
	GC2155_write_cmos_sensor(0xd2 , 0x28);
	...
	GC2155_write_cmos_sensor(0x73 , 0x30);
	...
	GC2155_write_cmos_sensor(0x28 , 0xd6); //20fps //4e2 
	GC2155_write_cmos_sensor(0x29 , 0x07); 
	GC2155_write_cmos_sensor(0x2a , 0xd0); //16fps //6d6
	GC2155_write_cmos_sensor(0x2b , 0x0a); 
	GC2155_write_cmos_sensor(0x2c , 0xbe); //12fps//7d0 george
...
UINT32 GC2155Open(void)
{
	...
	for(i=0;i<3;i++)
	{
		...
		else 
		{
			...
			//gc2155_Initialize_from_T_Flash();
		...
		{
			//GC2155_MPEG4_encode_mode = KAL_FALSE;
...
BOOL GC2155_set_param_banding(UINT16 para)
{
    switch (para)
    {
		...
		GC2155_write_cmos_sensor(0x28, 0xe0); //20fps 
		GC2155_write_cmos_sensor(0x29, 0x06);
		GC2155_write_cmos_sensor(0x2a, 0x80); //16fps 
		GC2155_write_cmos_sensor(0x2b, 0x08);
		GC2155_write_cmos_sensor(0x2c, 0x20); //16fps  
		GC2155_write_cmos_sensor(0x2d, 0x0b);
		GC2155_write_cmos_sensor(0x2e, 0x60); //8fps 
		GC2155_write_cmos_sensor(0xfe, 0x00); 
            break;
...
#ifdef DEBUG_SENSOR_GC2155
		printk("______%s______GC2155 YUV setting\n",__func__);
		return TRUE;
#endif
...
/*************************************************************************
  * FUNCTION
  * GC2155SetMaxFramerateByScenario
  *
  * DESCRIPTION
  ...
  *************************************************************************/
  ...
  UINT32 GC2155SetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) {
	kal_uint32 pclk;
	kal_int16 dummyLine;
...
/*************************************************************************
  * FUNCTION
  * GC2155GetDefaultFramerateByScenario
  *
  * DESCRIPTION
  * This function is for android4.4 kk
  * RETURNS
  ...
  *
  *************************************************************************/
UINT32 GC2155GetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 *pframeRate) 
{
