...
#include "gc0310yuv_Sensor.h"
#include "gc0310yuv_Camera_Sensor_para.h"
#include "gc0310yuv_CameraCustomized.h"
...
//#define DEBUG_SENSOR_GC0310//T_flash Tuning
...
kal_uint16 GC0310_read_cmos_sensor(kal_uint8 addr)
{
	...
	iReadRegI2C(&puSendCmd , 1, (u8*)&get_byte, 1, GC0310_WRITE_ID);
	
    return get_byte;
}


#ifdef DEBUG_SENSOR_GC0310
#define gc0310_OP_CODE_INI		0x00		/* Initial value. */
#define gc0310_OP_CODE_REG		0x01		/* Register */
#define gc0310_OP_CODE_DLY		0x02		/* Delay */
#define gc0310_OP_CODE_END	0x03		/* End of initial setting. */
...
} gc0310_initial_set_struct;

gc0310_initial_set_struct gc0310_Init_Reg[5000];

static u32 strtol(const char *nptr, u8 base)
{

	printk("gc0310___%s____\n",__func__); 

	u8 ret;
	if(!nptr || (base!=16 && base!=10 && base!=8))
	{
		printk("gc0310 %s(): NULL pointer input\n", __FUNCTION__);
		return -1;
	}
	for(ret=0; *nptr; nptr++)
	{
...
static u8 GC0310_Initialize_from_T_Flash()
{
	...
	fp = filp_open("/mnt/sdcard/gc0310_sd.txt", O_RDONLY , 0); 
	if (IS_ERR(fp)) 
	{ 
	...
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			gc0310_Init_Reg[i].op_code = gc0310_OP_CODE_REG;

			gc0310_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */

			gc0310_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		...
		else									/* DLY */
		{
			...
			curr_ptr += 4;	
			gc0310_Init_Reg[i].op_code = gc0310_OP_CODE_DLY;

			gc0310_Init_Reg[i].init_reg = 0xFF;
			gc0310_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
	...
	gc0310_Init_Reg[i].op_code = gc0310_OP_CODE_END;
	gc0310_Init_Reg[i].init_reg = 0xFF;
	gc0310_Init_Reg[i].init_val = 0xFF;
	i++;
	//for (j=0; j<i; j++)
	printk("gc0310 %x  ==  %x\n",gc0310_Init_Reg[j].init_reg, gc0310_Init_Reg[j].init_val);

	/* Start apply the initial setting to sensor. */
#if 1
	for (j=0; j<i; j++)
	{
		if (gc0310_Init_Reg[j].op_code == gc0310_OP_CODE_END)	/* End of the setting. */
		{
			printk("gc0310 REG OK -----------------END!\n");
		
			break ;
		}
		else if (gc0310_Init_Reg[j].op_code == gc0310_OP_CODE_DLY)
		{
			msleep(gc0310_Init_Reg[j].init_val);		/* Delay */
			printk("gc0310 REG OK -----------------DLY!\n");			
		}
		else if (gc0310_Init_Reg[j].op_code == gc0310_OP_CODE_REG)
		{
			GC0310_write_cmos_sensor(gc0310_Init_Reg[j].init_reg, gc0310_Init_Reg[j].init_val);
			printk("gc0310 REG OK!--------1--------REG(0x%x,0x%x)\n",gc0310_Init_Reg[j].init_reg, gc0310_Init_Reg[j].init_val);			
			printk("gc0310 REG OK!--------2--------REG(0x%x,0x%x)\n",gc0310_Init_Reg[j].init_reg, gc0310_Init_Reg[j].init_val);			
			printk("gc0310 REG OK!--------3--------REG(0x%x,0x%x)\n",gc0310_Init_Reg[j].init_reg, gc0310_Init_Reg[j].init_val);			
		}
		else
		{
			printk("gc0310 REG ERROR!\n");
		}
	}
#endif
	printk("gc0310 ___%s____ end\n",__func__); 
	return 1;	
}
...
static kal_uint32 GC0310_g_fPV_PCLK = 26;
...
void GC0310_Sensor_Init(void)
{
	...
	GC0310_write_cmos_sensor(0xfc, 0x0e); 
	GC0310_write_cmos_sensor(0xf2, 0x07); 
	GC0310_write_cmos_sensor(0xf3, 0x00);// output_disable 
	...
	GC0310_write_cmos_sensor(0x01, 0x0f);//06
	...
	GC0310_write_cmos_sensor(0x1c, 0x6c);//1c travis 20140929 
	GC0310_write_cmos_sensor(0x1e, 0x6b);
	GC0310_write_cmos_sensor(0x1f, 0x28);
	GC0310_write_cmos_sensor(0x20, 0x8b);//89 travis 20140801
	GC0310_write_cmos_sensor(0x21, 0x49);
	GC0310_write_cmos_sensor(0x22, 0xd0);//b0 travis 20140929
	...
	//GC0310_write_cmos_sensor(0x32, 0x04);//00  travis 20140929
	...
	GC0310_write_cmos_sensor(0x46, 0x02); //sync
	...
	//GC0310_write_cmos_sensor(0xa4, 0x80);//30 travis 20140929
	//GC0310_write_cmos_sensor(0xa5, 0x28);//30 travis 20140929
	//GC0310_write_cmos_sensor(0xaa, 0x30);//10 travis 20140929 				
...
*/
	...
	GC0310_write_cmos_sensor(0x20, 0xc0); //80
...
*/
	...
	GC0310_write_cmos_sensor(0x26, 0x6a);   //anti-flicker step [7:0]

	GC0310_write_cmos_sensor(0x27, 0x02);   //exp level 0  20fps
	GC0310_write_cmos_sensor(0x28, 0x12); 
	GC0310_write_cmos_sensor(0x29, 0x03);   //exp level 1  12.50fps
	GC0310_write_cmos_sensor(0x2a, 0x50); 
	GC0310_write_cmos_sensor(0x2b, 0x05);   //7.14fps
	GC0310_write_cmos_sensor(0x2c, 0xcc); 
	GC0310_write_cmos_sensor(0x2d, 0x07);   //exp level 3  5.55fps
	...
	GC0310_write_cmos_sensor(0x01, 0x00);
	GC0310_write_cmos_sensor(0x02, 0x00);
	GC0310_write_cmos_sensor(0x10, 0x00);
	GC0310_write_cmos_sensor(0x15, 0x00);
	GC0310_write_cmos_sensor(0xfe, 0x00);
	///////////////////OUTPUT//////////////////////
	GC0310_write_cmos_sensor(0xf3, 0xff);// output_enable
...
UINT32 GC0310GetSensorID(UINT32 *sensorID)
{
    ...
    do {
        *sensorID=((GC0310_read_cmos_sensor(0xf0)<< 8)|GC0310_read_cmos_sensor(0xf1));
        SENSORDB("[lsz]Read Sensor ID Fail = 0x%04x\n", *sensorID);
...
UINT32 GC0310Open(void)
{
	...
	for(i=0;i<3;i++)
	{
		...
		{
			SENSORDB("GC0310 Read Sensor ID Fail[open] = 0x%x\n", sensor_id); 
			return ERROR_SENSOR_CONNECT_FAIL;
		}
	}
	
	SENSORDB("GC0310_ Sensor Read ID OK \r\n");
	GC0310_Sensor_Init();
#ifdef DEBUG_SENSOR_GC0310  
		struct file *fp; 
		mm_segment_t fs; 
		...
		{ 
			fromsd = 0;   
			printk("gc0310 open file error\n");
		} 
		else 
		{
			fromsd = 1;
			printk("gc0310 open file ok\n");
		...
		if(fromsd == 1)
		{
			printk("gc0310 open from t!\n");
			GC0310_Initialize_from_T_Flash();
		}
		else
		{
			printk("gc0310 open not from t!\n");		
		}
...
BOOL GC0310_set_param_banding(UINT16 para)
{
	switch (para)
	{
		...
		case AE_FLICKER_MODE_60HZ:
			GC0310_write_cmos_sensor(0xfe, 0x00); 
			GC0310_write_cmos_sensor(0x05, 0x02); 	
			GC0310_write_cmos_sensor(0x06, 0x60); 
			GC0310_write_cmos_sensor(0x07, 0x00);
			GC0310_write_cmos_sensor(0x08, 0x58);
			
			GC0310_write_cmos_sensor(0xfe, 0x01);
			GC0310_write_cmos_sensor(0x25, 0x00);   //anti-flicker step [11:8]
			GC0310_write_cmos_sensor(0x26, 0x60);   //anti-flicker step [7:0]
			
			GC0310_write_cmos_sensor(0x27, 0x02);   //exp level 0  20fps
			GC0310_write_cmos_sensor(0x28, 0x40); 
			GC0310_write_cmos_sensor(0x29, 0x03);   //exp level 1  13fps
			GC0310_write_cmos_sensor(0x2a, 0x60); 
			GC0310_write_cmos_sensor(0x2b, 0x06);   //exp level 2  7.5fps
			GC0310_write_cmos_sensor(0x2c, 0x00); 
			GC0310_write_cmos_sensor(0x2d, 0x08);   //exp level 3  5.45fps
			GC0310_write_cmos_sensor(0x2e, 0x40); 
			GC0310_write_cmos_sensor(0xfe, 0x00);
		break;
		default:
		return FALSE;
...
BOOL GC0310_set_param_exposure(UINT16 para)
{
	...
	switch (para)
	{
		...
		case AE_EV_COMP_n20:
			GC0310_write_cmos_sensor(0xfe, 0x01);
			GC0310_write_cmos_sensor(0x13, 0x28);
			GC0310_write_cmos_sensor(0xfe, 0x00);
		break;
		...
		case AE_EV_COMP_10:			
			GC0310_write_cmos_sensor(0xfe, 0x01);
			GC0310_write_cmos_sensor(0x13, 0x40);
			GC0310_write_cmos_sensor(0xfe, 0x00);
		break;
		
		case AE_EV_COMP_20:
			GC0310_write_cmos_sensor(0xfe, 0x01);
			GC0310_write_cmos_sensor(0x13, 0x48);
			GC0310_write_cmos_sensor(0xfe, 0x00);
		break;
        ...
        UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    RETAILMSG(1, (_T("gaiyang GC0310FeatureControl FeatureId=%d\r\n"), FeatureId));
