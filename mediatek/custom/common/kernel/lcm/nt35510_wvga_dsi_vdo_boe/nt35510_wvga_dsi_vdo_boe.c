...
#define LCM_ID_NT35510 (0x5512) //(0x5510)  change by jst for r6372_alx_a26
...
static unsigned int lcm_compare_id(void)
{
	...
	//array[0] = 0x00083700; //del by jst for debug
	array[0] = 0x00023700;
...
#if defined(BUILD_LK)
	printf("[nt35510 u-boot] %s,  buffer[0]=%x,buffer[1]=%x,id = 0x%x \n", __func__,buffer[0],buffer[1], id);
#else
  printk("[nt35510 kernel] %s,  buffer[0]=%x,buffer[1]=%x,id = 0x%x \n", __func__,buffer[0],buffer[1], id);
#endif

   //return (LCM_ID_NT35510 == id) ? 1 : 0;
   if((0x5510 == id)||(0x5512 == id)||(0x0 == id)) //change by jst for alx_a26 debug
       return 1;
   else
       return 0;
...
LCM_DRIVER nt35510_wvga_dsi_vdo_boe_lcm_drv = 
{
  .name		= "nt35510_wvga_dsi_vdo_boe",
