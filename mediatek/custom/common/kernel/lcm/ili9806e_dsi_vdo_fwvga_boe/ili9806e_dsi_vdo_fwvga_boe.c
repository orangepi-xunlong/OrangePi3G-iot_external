...
{
    lcm_util.set_gpio_mode(GPIO_DISP_LRSTB_PIN, GPIO_DISP_LRSTB_PIN_M_GPIO);
    lcm_util.set_gpio_dir(GPIO_DISP_LRSTB_PIN, GPIO_DIR_OUT);
	MDELAY(2);
	LCM_LOG("tek.xing lcm_init\n");
...
#if defined(BUILD_LK)
	printf("ili9806 id=%x, %x %x %x %x\n", id, id1, id2, id3, id4);
#else
	printk("ili9806 id=%x, %x %x %x %x\n", id, id1, id2, id3, id4);
#endif
...
LCM_DRIVER ili9806e_dsi_vdo_fwvga_boe_lcm_drv= 
{
    .name			= "ili9806e_dsi_vdo_fwvga_boe",
