...
{ 
	...
	params->dsi.vertical_backporch					= 30;//10
	params->dsi.vertical_frontporch					= 20;//11
	params->dsi.vertical_active_line				= FRAME_HEIGHT; 

	params->dsi.horizontal_sync_active				= 50;//14
	params->dsi.horizontal_backporch				= 60;//80
	params->dsi.horizontal_frontporch				= 60;
	params->dsi.horizontal_active_pixel				= FRAME_WIDTH;

	params->dsi.PLL_CLOCK=207;//172;//185;//213;//208
#endif	
}
...
static unsigned int lcm_compare_id(void)
{
	...
	read_reg_v2(0xA1, &buffer[0], 2);
        ...
        id=(id_high<<8)| id_low;
		#ifdef BUILD_LK
		printf("st7701s_fwvga_dsi_vdo_lcm_drv %s:0x%2x,0x%2x,0x%2x,0x%2x id=0x%x\n", __func__,buffer[0],buffer[1],buffer[2],buffer[3], id);
	#else
		printk("st7701s_fwvga_dsi_vdo_lcm_drv %s:0x%2x,0x%2x,0x%2x,0x%2x id=0x%x\n", __func__,buffer[0],buffer[1],buffer[2],buffer[3], id);
	#endif 
    return ((LCM_ID_ST7701S == id) ? 1:0);	
...
#ifdef BUILD_LK
    printf("uboot:st7701s_lcm_setbacklight mapped_level = %d,level=%d\n",mapped_level,level);
#else
    printk("kernel:st7701s_lcm_setbacklight mapped_level = %d,level=%d\n",mapped_level,level);
#endif
...
// ---------------------------------------------------------------------------
LCM_DRIVER st7701s_fwvga_dsi_vdo_boe_lcm_drv = 
{
	.name		= "st7701s_fwvga_dsi_vdo_boe",
