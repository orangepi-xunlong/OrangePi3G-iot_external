...
static int ltr559_ps_enable(int gainrange)
{
   ...
   error = ltr559_i2c_write_reg(LTR559_PS_LED, 0x1f); 
	if(error<0)
    {
...
static int ltr559_local_init(void)
{
	struct alsps_hw *hw = ltr_get_cust_alsps_hw();
...
{
	APS_FUN();
	i2c_register_board_info(1, &i2c_ltr559, 1);
#if defined(MTK_AUTO_DETECT_ALSPS)
