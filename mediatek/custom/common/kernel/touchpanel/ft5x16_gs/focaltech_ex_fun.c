...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x64))
	{
		...
		{
			dev_err(&client->dev, "%s: $s FW length error\n", __func__, fts_updateinfo_curr.FTS_NAME);
			return -EIO;
		}
		pbt_buf = CTPM_FW;
		...
		i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW));
		if (i_ret != 0)
			dev_err(&client->dev, "%s: $s upgrade failed. err.\n",__func__, fts_updateinfo_curr.FTS_NAME);
	}
...
int fts_5x26_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if(i_ret < 0)
		{
			DBG("failed to Switch HidtoI2c Protocol ! \n");
			continue;
		}
	...
	DBG("Step 8: Disable Write Flash\n");
    	auc_i2c_write_buf[0] = 0x04;
    	fts_i2c_Write(client, auc_i2c_write_buf, 1);
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	u8  	bt_ecc;
	int	i_ret;
	int	fw_filenth = sizeof(CTPM_FW);//sizeof(CTPM_FW);
