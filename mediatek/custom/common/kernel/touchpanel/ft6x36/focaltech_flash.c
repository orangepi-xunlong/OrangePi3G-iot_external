...
#ifdef TP_Multi_Touch_Screen_Compatible_Upgrade

unsigned int CTPM_FW_SIZE = 0;
unsigned char *CTPM_FW;
#if 0
unsigned char CTPM_FW_HS[] = /*TP厂1的i文件存储数组名*/
{
	#include "HS_HS040290A_4011_0x01_Achen_20150526_app.i"
};
unsigned char CTPM_FW_RN[] = /*TP厂2的i文件存储数组名*/
{
	#include "FT6236_RN_XC534_ZX4011_ID012_Ver0XA1_20150529_app.i"
};
unsigned char CTPM_FW_YJ[] = /*TP厂3的i文件存储数组名*/
{
	#include "YJ_FT6206_Each4001_ZeXin_4001_ID0X80_Ver0XB1_20150120_app.i"
};
unsigned char CTPM_FW_YUDA[]= /*TP厂4的i文件存储数组名*/
{
    #include "YD_FT6206_Each4001_ZeXin_4001_ID0X80_Ver0XB1_20150120_app.i"
};
#endif	
unsigned char CTPM_FW_DEFAULT[] = /*默认TP厂的i文件存储数组名*/
{
	#include "QK_601_ft_app.i"
};
int CheckIDforTP(unsigned int Vendor_ID)/*根据屏厂ID号自动将i文件复制给CTPM_FW数组，实现兼容升级*/
{
	printk("mtk_tpd Vendor_ID = 0x%x\n",Vendor_ID);
    #ifdef TP_Multi_Touch_Screen_Compatible_Upgrade
	/*
	switch(Vendor_ID)
	{
	case VENDOR_ID_FORM_HUASHANG:
		CTPM_FW = CTPM_FW_HS;
		CTPM_FW_SIZE = sizeof(CTPM_FW_HS);
		printk("CheckIDforTP:mtk_tpd VENDOR_ID_FORM_HUASHANG 0x%x\n",CTPM_FW_SIZE);
		break;
	case VENDOR_ID_FORM_RONGNNA:
		CTPM_FW = CTPM_FW_RN;
		CTPM_FW_SIZE = sizeof(CTPM_FW_RN);
		printk("mtk_tpd VENDOR_ID_FORM_RONGNNA 0x%x\n",CTPM_FW_SIZE);
		break;
	case VENDOR_ID_FORM_YUDA:
		CTPM_FW = CTPM_FW_YUDA;
		CTPM_FW_SIZE = sizeof(CTPM_FW_YUDA);
		printk("mtk_tpd VENDOR_ID_FORM_YUDA 0x%x\n",CTPM_FW_SIZE);
		break;
	case VENDOR_ID_FORM_YEJI:
		CTPM_FW = CTPM_FW_YJ;
		CTPM_FW_SIZE = sizeof(CTPM_FW_YJ);
		printk("mtk_tpd VENDOR_ID_FORM_YEJI 0x%x\n",CTPM_FW_SIZE);
		break;
	default:
		CTPM_FW = CTPM_FW_DEFAULT;
		CTPM_FW_SIZE = sizeof(CTPM_FW_DEFAULT);
		printk("mtk_tpd CTPM_FW_DEFAULT 0x%x\n",CTPM_FW_SIZE);
		return 0;
	}
	*/
	CTPM_FW = CTPM_FW_DEFAULT;
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	u8  	bt_ecc;
	int	i_ret;
	int	fw_filenth = CTPM_FW_SIZE;//sizeof(CTPM_FW);
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = CTPM_FW_SIZE;//sizeof(CTPM_FW);

	/*judge the fw that will be upgraded
	* if illegal, then stop upgrade and return.
	*/
	
	printk("mtk_tpd fts_updateinfo_curr.CHIP_ID 0x%x\n",fts_updateinfo_curr.CHIP_ID);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	//	) 
	printk("tpd fts_ctpm_auto_upgrade uc_tp_fm_ver = %x,uc_host_fm_ver = %x\n",uc_tp_fm_ver,uc_host_fm_ver);
