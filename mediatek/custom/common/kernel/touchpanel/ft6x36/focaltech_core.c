...
***********************************************************************/
 ...
 {	 
	...
	#endif
#ifdef TPD_POWER_SOURCE_CUSTOM_BAK
	hwPowerOn(TPD_POWER_SOURCE_CUSTOM_BAK, VOL_2800, "TP");
#endif
    ...
    fts_init_gpio_hw();
	/****************\D7\F6MS2133\D3\EBFT\B5ļ\E6\C8\DDʶ\B1\F0******************************************/
	if((i2c_smbus_read_i2c_block_data(fts_i2c_client, 0x00, 1, &data))< 0)	
	{		
...
#ifdef TP_Multi_Touch_Screen_Compatible_Upgrade  /*\B6\E0\BC\D2TP\B3\A7i\CEļ\FE\C9\FD\BC\B6\BC\E6\C8ݴ\A6\C0\ED*/
	{
		int auto_upgrade;
		printk("********************Enter CTP Auto Upgrade:0001********************\n");
		uc_reg_addr=FTS_REG_VENDOR_ID;
		fts_i2c_write(fts_i2c_client, &uc_reg_addr, 1);
		fts_i2c_read(fts_i2c_client, &uc_reg_addr, 0, &uc_reg_value, 1);
		auto_upgrade = CheckIDforTP(uc_reg_value);	
		if(auto_upgrade)
		{
