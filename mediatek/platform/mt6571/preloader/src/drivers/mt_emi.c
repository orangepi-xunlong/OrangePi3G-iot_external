...
#define DRAM_BASE_Address       ((volatile UINT32P)(0x80001000))
static void Dummy_DRAM_Write(void)
{
	print("[EMI] Dummy wirte to 0x80001000\n");
	*DRAM_BASE_Address = 0x12345678;
}

static void set_emi_clk_26M(U32 enable)
{
	U32 EMI_reg;
	EMI_reg = *((volatile UINT32P)(0x10000000)); // CLK_SEL_0
	if (enable == 1)
	{
		*((volatile UINT32P)(0x10000000)) = EMI_reg & 0xFFFFFFEF; // clean bit4 to 26Mhz	
	}
	else if (enable == 0)
	{
		*((volatile UINT32P)(0x10000000)) = EMI_reg | 0x10; // set bit4 to return original freq.	
	}
	gpt_wait_us_dsb(10);	
}
...
int get_board_dram_info(void)
{
    ...
    *EMI_DRCT &= (~0x1);    
	...
	MR8_r0 = 	get_mode_reg(0, 8);
	MR32_r0 = 	get_mode_reg(0, 32);
	MR40_r0 = 	get_mode_reg(0, 40);
	MR5_r0 = 	get_mode_reg(0, 5);
...
#if !defined(CFG_MEM_PRESERVED_MODE)  //Ignore DRAM initialization
	/**
	   * LPDDR Initial Flow.
	   */
...
static void init_pcdram3(EMI_SETTINGS *emi_setting)
{
	...
	*EMI_GENA |= 0x0A;
	...
	/**
	   * At PCDDR3-16 and PCDDR3-8, DCMDQ_DEPTH of EMI_GENA must set to 3 level,
	   * Otherwise, set to 6 level
	   */

	CONN_dram_type = ((emi_setting->EMI_CONN_value) & 0x00070000) >>16 ;
	if (CONN_dram_type == 0x6)
	{
		// set DCMDQ_DEPTH to 3 level
		*EMI_GENA |= 0xC0;
	}
...
static int mt_get_mdl_number (void)
{
	...
	//load mdl number from EMI controller reserved bits: EMI_GENA bit 20~23
    mdl_number = (*EMI_GENA & 0x00F00000) >> 20;
	found = 1;
	return mdl_number;
#endif // #if defined(CFG_MEM_PRESERVED_MODE)
...
void __EMI_InitializeLPDDR( EMI_SETTINGS *emi_setting )
{
...
#if defined(CFG_MEM_PRESERVED_MODE)  //force DRAM enter self-refresh
        /* Disable EMI dummy read */
        *EMI_DRCT = 0; //Add by IvanTseng
	...
	gpt_wait_us_dsb(1);
        ...
        *EMI_DQSI &= ~(0x100000FF);        /* Suggest by Fumin */
	...
	gpt_wait_us_dsb(1);
       ...
       //switch EMI clock source from 13Mhz to target freq.  /* EMI and bus clock */
	 i = DRV_Reg32(CLK_SEL_0);
	 i |= 0x10;
	DRV_WriteReg32(CLK_SEL_0,  i);
	...
	*/
	...
	//re-boot, should reset permission setting to avoid some module can't use memory
	if ( (0 != *EMI_MPUA) | (0 != *EMI_MPUB) |  (0 != *EMI_MPUC) |  (0 != *EMI_MPUD) |
	     (0 != *EMI_MPUE) | (0 != *EMI_MPUF) |  (0 != *EMI_MPUG) |  (0 != *EMI_MPUH) )
	{	
		set_emi_mpu_key();
	...
	*EMI_DRCT = 0x0;
	 ...
	 *(volatile kal_uint32*)EMI_PPCT = 0xFF9FBF0A;
...
#if defined(CFG_MEM_PRESERVED_MODE)  //force DRAM exit self-refresh
...
kal_int32 __EMI_DataTrackingMbistTestCore(kal_int32 algo_sel, kal_int32 addr_scramble_sel, kal_int32 data_scramble_sel)
{
    ...
    *EMI_MBISTB = 0xFFFF0000 | ((0x0000A55A) >> (algo_sel + addr_scramble_sel + data_scramble_sel));
    ...
    // memory preserve mode only can use 0x80020000 ~ 0x80100000
    /**
...
int __EMI_EnablePerformancePowerControl(EMI_SETTINGS *emi_setting)
{
    if ((emi_setting->type & 0xF) == 1)
    {
		/**
		   ...
		   */
		//*(volatile kal_uint32*)EMI_PPCT=0xFF9FBF0A;  // must disable memory preserve mode

		*(volatile kal_uint32*)EMI_SLCT=0x3F183F10;
...
#define DDR3_AUTO_DETECT_SUPPORT
#ifdef DDR3_AUTO_DETECT_SUPPORT
int mt_ddr3_autodetect_enable (EMI_SETTINGS *emi_setting)
{
	unsigned int enbale = 0x00;//0 : disable auto detect ; 1: enable auto detect
	
	if((*((volatile unsigned int *)0x10009184) & 0x00004000) && (mt_get_dram_type() == PCDDR3))
	{
		if(((emi_setting->EMI_CONN_value & 0x00770000)>>16) == 0x77)
		{
			enbale = 0x01;
		}
	}
...
int mt_ddr3_datacheck( EMI_DATA_TRAIN_REG_t* pResult, int rank_idx )
{
#define ___EMIDATATRAIN_MBIST_DATATRAIN__
#define DEBUG_MODE 0
#define DQ_IN_DLY_TAPS 0x0E
...
#define DQSI_TUNING_WINDOW_SIZE_THRESHOLD 6
	     ...
	     gpt_wait_us_dsb(1);
            *EMI_CALP &= 0xFFFFFEFF;
            ...
            if(lbound_finding==1 && test_result == 0)
            {
                ...
                #if DEBUG_MODE
                dbg_print("Find L Bound1 (0x%x, 0x%x, 0x%x)\n\r", dqy_in_del, bytex_dly_mod, bytex_setup_mod);
                #endif
                dqy_in_del_start = dqy_in_del;
                bytex_dly_mod_start = bytex_dly_mod;
                bytex_setup_mod_start = bytex_setup_mod;
            ...
            else if(lbound_finding==0 && test_result != 0 )
            {
                ...
                if (dwnd_size <= DATA_TUNING_STEP)  //ignore this window, continue find window
                {
                    lbound_finding = 1;
                    #if DEBUG_MODE
                    dbg_print("Ignore this window1 (0x%x, 0x%x, 0x%x)\n\r", dqy_in_del, bytex_dly_mod, bytex_setup_mod);
                    #endif
                }
                else
                {
                    #if DEBUG_MODE
                    dbg_print("Find R Bound1 (0x%x, 0x%x, 0x%x)\n\r", dqy_in_del, bytex_dly_mod, bytex_setup_mod);
                    #endif
        ...
        for(bytex_setup_mod=0; bytex_setup_mod<=DATA_SETUP_TAPS; bytex_setup_mod+=DATA_TUNING_STEP)
        {
	     ...
	     gpt_wait_us_dsb(1);
            *EMI_CALP &= 0xFFFFFEFF;
            ...
            {
                #if DEBUG_MODE
                dbg_print("Find L Bound2 (0x%x, 0x%x, 0x%x)\n\r", dqy_in_del, bytex_dly_mod, bytex_setup_mod);
                #endif
            ...
            else if(lbound_finding==0 && test_result != 0 )
            {
                ...
                if (dwnd_size <= DATA_TUNING_STEP)  //ignore this window, continue find window
                {
                    lbound_finding = 1;
                   #if DEBUG_MODE
                    dbg_print("Ignore this window2 (0x%x, 0x%x, 0x%x)\n\r", dqy_in_del, bytex_dly_mod, bytex_setup_mod);
                   #endif
                }
                else
                {
                    #if DEBUG_MODE
                    dbg_print("Find R Bound2 (0x%x, 0x%x, 0x%x)\n\r", dqy_in_del, bytex_dly_mod, bytex_setup_mod);
                    #endif
	     ...
	     gpt_wait_us_dsb(1);
            *EMI_CALP &= 0xFFFFFEFF;
            ...
            if(lbound_finding==1 && test_result == 0 )
            {
                ...
                #if DEBUG_MODE
                dbg_print("Find L Bound3 (0x%x, 0x%x, 0x%x)\n\r", dqy_in_del, bytex_dly_mod, bytex_setup_mod);
                #endif
            ...
            else if(lbound_finding==0 && test_result != 0 )
            {
                ...
                if (dwnd_size <= DATA_TUNING_STEP)  //ignore this window, continue find window
                {
                    lbound_finding = 1;
                    #if DEBUG_MODE
                    dbg_print("Ignore this window3 (0x%x, 0x%x, 0x%x)\n\r", dqy_in_del, bytex_dly_mod, bytex_setup_mod);
                    #endif
            }
                else
                {
                    #if DEBUG_MODE
                    dbg_print("Find R Bound3 (0x%x, 0x%x, 0x%x)\n\r", dqy_in_del, bytex_dly_mod, bytex_setup_mod);
                    #endif
    ...
    window_found:
        ...
        else
        {
            ...
            dbg_print("[EMI FY]Window size = %d, DQSI=0x%x\n\r", dwnd_size, dqsix_dlysel);
            __EmiDataTrainRegWrite( &REG_BAK, rank_idx );    /*Restore Register*/
			return  b_wnd_found;
        }
    ...
    //#if DEBUG_MODE
     print("[EMI FY]DQSI=0x%x,(0x%x,0x%x)\n", DQSI_center, DQSI_start, DQSI_end);
	 print("[EMI FY]Window=%d \n", dwnd_size);
    //#endif
...
void mt_ddr3_datacheck1(void)
{
	kal_uint32 EMI_CONJ_org;
	kal_uint32 EMI_CONK_org;
	kal_uint32 EMI_CONL_org;
	kal_uint32 EMI_CONN_org;
	kal_uint32 EMI_GEND_org;
	kal_uint32 EMI_GENA_org;

	kal_uint32 EMI_CONJ_new;
	kal_uint32 EMI_CONK_new;
	kal_uint32 EMI_CONL_new;
	kal_uint32 EMI_CONN_new;
	kal_uint32 EMI_GEND_new;
	kal_uint32 EMI_GENA_new;

	EMI_DATA_TRAIN_REG_t    DATA_TRAIN_RESULT_REG;

	EMI_CONJ_org =	*EMI_CONJ;
	EMI_CONK_org =	*EMI_CONK;
	EMI_CONL_org =	*EMI_CONL;
	EMI_CONN_org =	*EMI_CONN;
	EMI_GEND_org =	*EMI_GEND;
	EMI_GENA_org =	*EMI_GENA;
			
	//===================================================
	EMI_CONN_new = (EMI_CONN_org & 0xFF88FFFF) | 0x00470000;
	*EMI_CONN = EMI_CONN_new;
	if(mt_ddr3_datacheck(&DATA_TRAIN_RESULT_REG, 0) == 0)	
	{
		EMI_CONN_new = (EMI_CONN_org & 0xFF88FFFF) | 0x00460000;
		*EMI_CONN = EMI_CONN_new;
		if ((((*EMI_CONN) & 0x00070000) >>16) == 0x06)
		{
			// set DCMDQ_DEPTH to 3 level
			*EMI_GENA |= 0xC0;
		}
		...
		if(mt_ddr3_datacheck(&DATA_TRAIN_RESULT_REG, 0) == 0)	
		{
			*EMI_CONN = EMI_CONN_org;
			*EMI_GENA = EMI_GENA_org;
		}
		else
		{
			EMI_CONN_new = (EMI_CONN_org & 0xFF88FFFF) | 0x00660000;
			*EMI_CONN = EMI_CONN_new;				
		}
	}
	else
	{
		EMI_CONN_new = (EMI_CONN_org & 0xFF88FFFF) | 0x00570000;
		*EMI_CONN = EMI_CONN_new;			
	}		
	
}

void mt_ddr3_datacheck2(void)
{
	unsigned int i;
	...
	*((volatile unsigned int *)start_addr) = u4pattern;
	...
	for(i = 0; i < 3; i++)
	{
		test_addr = start_addr + (0x10000000 << i);
		if(*((volatile unsigned int *)test_addr) == u4pattern)
		{
			*((volatile unsigned int *)start_addr) = (~u4pattern);
			if(*((volatile unsigned int *)test_addr) == (~u4pattern))
			{
	...
	if(i > 2)	i = 2;
	emi_settings[0].DRAM_RANK_SIZE[0] = 0x10000000 << i;
	if ((((*EMI_CONN) & 0x00070000) >>16) == 0x06)
	{
		*EMI_CONN = ((*EMI_CONN & 0xFF8FFFFF) | ((i + 4) << 20));
	}
	else
	{
		*EMI_CONN = ((*EMI_CONN & 0xFF8FFFFF) | ((i + 3) << 20));
	}			
...
void mt_set_emi (void)
{
   ...
   #ifdef DDR3_AUTO_DETECT_SUPPORT
   if(mt_ddr3_autodetect_enable(emi_set))
   {
		mt_ddr3_datacheck1();
   }
...
#if !defined(CFG_MEM_PRESERVED_MODE) 
	// save mdl number : index to EMI controller reserved bits: EMI_GENA bit 20~23
    EMI_reg = *EMI_GENA & 0xFF0FFFFF;
	*EMI_GENA = EMI_reg | ((index & 0xF) << 20);   
#endif // #if !defined(CFG_MEM_PRESERVED_MODE)
