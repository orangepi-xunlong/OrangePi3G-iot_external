...
startOne(SensorParam const & rSensorParam)
{
    ...
    mbCCmode = rSensorParam.u4RawType == 1 ? 1 : 0;
...
sendCommand(MINT32 cmd, MINT32 arg1, MINT32 arg2, MINT32 arg3)
{
    ...
    switch(cmd)
    {
   	case ECamShot_CMD_START_GY_BEAUTY:
		mGYEnabledBeauty = arg1;
		break;
	case ECamShot_CMD_SET_GY_BEAUTY_SMOOTH:
		mGYBeautySmooth = arg1;
		break;
	case ECamShot_CMD_SET_GY_BEAUTY_WHITENING:
		mGYBeautyWhitening = arg1;
		break;
    }
    #endif
...
createSensorRawImg(SensorParam const & rSensorParam, ImgBufInfo const & rRawImgBufInfo)
{
    ...
    ret = ret
            && p3AObj->setIspProfile( mbCCmode ? EIspProfile_NormalCapture_CC :
...
rotatePostview(ImgBufInfo const & rSrcImgBufInfo, MINT32 const u4Rot, MINT32 const u4Flip, ImgBufInfo const & rDstImgBufInfo )
{
    ...
    {   //because simager will swap the w/h
...
createJpegImg(ImgBufInfo const & rSrcImgBufInfo, JpegParam const & rJpgParm, MUINT32 const u4Rot, MUINT32 const u4Flip, ImgBufInfo const & rJpgImgBufInfo, MUINT32 & u4JpegSize)
{
    ...
    //
    #ifdef RY_GANGYUN_CAMERA_BEAUTY
	if (mGYEnabledBeauty && u4JpegSize == 1){
		 #if 0
		 char fileName[256] = {'\0'};
		 sprintf(fileName, "/storage/sdcard0/%dx%d_in.yuv",  rSrcImgBufInfo.u4ImgWidth, rSrcImgBufInfo.u4ImgHeight);
		 MtkCamUtils::saveBufToFile(fileName, reinterpret_cast<MUINT8*>( rSrcImgBufInfo.u4BufVA), rSrcImgBufInfo.u4BufSize);
		 #endif
		 
		unsigned int m_handle = 0;

		m_handle = GYBeautyInit(0,  rSrcImgBufInfo.u4ImgWidth,  rSrcImgBufInfo.u4ImgHeight,  YUV_422); 

		MY_LOGE("gangyun tech:Shot:GYBeautyInit=%d,width=%d,height=%d", m_handle, rSrcImgBufInfo.u4ImgWidth, rSrcImgBufInfo.u4ImgHeight);
		if (m_handle>0){
                         mGYBeautySmooth =  (int)mGYBeautySmooth*1.2;
                         if(mGYBeautySmooth>200){
                             mGYBeautySmooth = 200;
                         }
                         mGYBeautyWhitening =  (int)mGYBeautyWhitening*1.2;
                         if(mGYBeautyWhitening>100){
                             mGYBeautyWhitening = 90;
                         }
			 GYBeautySetSmooth( mGYBeautySmooth );
			 GYBeautySetWhitening( mGYBeautyWhitening );

			 unsigned char* puJpegBuf = (unsigned char*)rSrcImgBufInfo.u4BufVA;
			 GYBeautySkinFace( puJpegBuf );
		...
		sprintf(fileName2, "/storage/sdcard0/%dx%d_out.yuv",  rSrcImgBufInfo.u4ImgWidth, rSrcImgBufInfo.u4ImgHeight);
		MtkCamUtils::saveBufToFile(fileName2, reinterpret_cast<MUINT8*>( rSrcImgBufInfo.u4BufVA), rSrcImgBufInfo.u4BufSize);
		#endif
	}
...
allocImgMem(char const* const pszName, EImageFormat const eFmt, MUINT32 const u4Width, MUINT32 const u4Height, IMEM_BUF_INFO & rMem)
{
    ...
    // add 4 bytes to IMGI buffer for tile driver padding
    if (strcmp("SensorRaw",pszName) == 0)
    {
        MY_LOGD("add 4 bytes to IMGI buffer for tile driver padding, %d=>%d", u4BufSize, u4BufSize+4);
        u4BufSize += 4;
    }
...
queryJpegImgBufInfo()
{
    ...
    ImgBufInfo rImgBufInfo; 
    if ( mRawMem.size > queryImgBufSize(eImgFmt_JPEG, u4Width, u4Height) )
    {
