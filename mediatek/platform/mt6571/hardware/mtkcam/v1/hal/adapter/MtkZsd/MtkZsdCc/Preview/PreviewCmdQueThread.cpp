...
init()
{
    ...
    {
        MY_LOGD("aaaa: isPass1DispOutNeeded == 1");
    ...
    {
        MY_LOGD("aaaa: isPass1DispOutNeeded == 0");
        mu4Pass1Port = eID_Pass1Out;
    }
...
start()
{
    ...
    getHw()->setCamMode((MUINT32)eAppMode_ZsdMode);
...
updateOne()
{
   ...
   #ifdef RY_GANGYUN_CAMERA_BEAUTY
	     //MY_LOGE("gangyun tech:Preview:imageFormat=%s,bufsize=%d,mBeauty=%d", dispNode.getImgBuf()->getImgFormat().string(), dispNode.getImgBuf()->getBufSize(), mGYEnabledBeauty);
   	     if (dispNode.getImgBuf() != 0 && mGYEnabledBeauty){
		    //dumpImg((MUINT8*)(dispNode.getImgBuf()->getVirAddr()),  dispNode.getImgBuf()->getBufSize(),  "pass2_dispo", "yuv",  mFrameCnt);
		     if ( mGYBufSize != dispNode.getImgBuf()->getBufSize() )
		     	{
		     		MY_LOGE("gangyun tech:Preview Size Changed=%d to %d",  mGYBufSize, dispNode.getImgBuf()->getBufSize() );
		     		getParamsMgr()->getPreviewSize(&mGYPreviewWidth,  &mGYPreviewHeight);
				if (mGYBeautyHandle > 0)
				{
					GYBeautyUnInit();
