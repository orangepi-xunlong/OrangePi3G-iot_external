...
#ifdef RY_GANGYUN_CAMERA_BEAUTY
#include <mtkcam/gybeauty/beautify.h>
#endif
//
...
//
#if defined(MTK_CAM_VSS_SUPPORT)
#define VSS_ENABLE      (1)
#else
#define VSS_ENABLE      (0)
#endif
...
public:
    ...
    bool init(ACDK_SCENARIO_ID_ENUM scenarioId) 
    {
        ...
        IhwScenario* pHwScenario = IhwScenario::createInstance(
                                                    ...
                                                    meSensorDev,
                                                    mSensorBitOrder); 
        if( scenarioId == ACDK_SCENARIO_ID_CAMERA_PREVIEW )
        {
            pHwScenario->getHwValidSize(eID_Pass1Out,mu4MemOutW,mu4MemOutH);
            mbResized = MFALSE;
        }
        else if( scenarioId == ACDK_SCENARIO_ID_VIDEO_PREVIEW )
        {
            const MBOOL tryResize = MTRUE;
            pHwScenario->getHwValidSize(eID_Pass1Out,mu4MemOutW,mu4MemOutH,30,tryResize);
            if( mu4MemOutW != mu4TgOutW )
            {
                mbResized = MTRUE;
            }
        }
...
public:    
    ...
    MBOOL               isResized()         const   { return mbResized; }
...
public:
   ...
   #ifdef RY_GANGYUN_CAMERA_BEAUTY
    virtual void startGYBeauty	(int isEnabled); 
    virtual void setGYBeautySmooth(int value); 
    virtual void setGYBeautyWhitening(int value); 
    #endif //gangyun tech
...
private:
    ...
    #endif
	#ifdef RY_GANGYUN_CAMERA_BEAUTY
    int				mGYEnabledBeauty; 
    unsigned int	mGYBeautyHandle; 
    int				mGYPreviewWidth;
    int				mGYPreviewHeight;
    int				mGYBeautySmooth;
    int 				mGYBeautyWhitening;
    int 				mGYMemLock;
    unsigned char*		mGYCpiedBuf;
    size_t				mGYBufSize;
   #endif
};
...
start()
{
    ...
    mbResized = mSensorInfo.isResized();
    MY_LOGD("resize %d", mbResized);
    ...
    getHw()->setCamMode((MUINT32)eAppMode_DefaultMode);
    ...
    #ifdef RY_GANGYUN_CAMERA_BEAUTY
	if (mGYEnabledBeauty){
			getParamsMgr()->getPreviewSize(&mGYPreviewWidth,  &mGYPreviewHeight);
			mGYBeautyHandle = GYBeautyInit(1, mGYPreviewWidth,  mGYPreviewHeight,  YUV_420); 
			MY_LOGE("gangyun tech:Preview:GYBeautyInit=%d,width=%d,height=%d", mGYBeautyHandle,  mGYPreviewWidth,  mGYPreviewHeight);
			if (mGYBeautyHandle>0){
				GYBeautySetSmooth(mGYBeautySmooth);
				GYBeautySetWhitening(mGYBeautyWhitening);
			}
	}
    #endif
...
stop()
{
    ...
    if (mpHwScenario != NULL)
    {
        if ( !mpHwScenario->stop() )
        {
            MY_LOGE("HwScenario stop fail");
            ret = false;        
        }
...
updateOne()
{
   ...
   #ifdef RY_GANGYUN_CAMERA_BEAUTY
   	     if (dispNode.getImgBuf() != 0 && mGYEnabledBeauty){
		     if ( mGYBufSize != dispNode.getImgBuf()->getBufSize() )
		     	{
		     		getParamsMgr()->getPreviewSize(&mGYPreviewWidth,  &mGYPreviewHeight);
				if (mGYBeautyHandle > 0)
				{
				...
				mGYBufSize = dispNode.getImgBuf()->getBufSize();
		     	}
		     if (mGYBeautyHandle == 0){
				    ...
				    if (mGYBeautyHandle>0){
						GYBeautySetSmooth(mGYBeautySmooth);
						GYBeautySetWhitening(mGYBeautyWhitening);
						mGYCpiedBuf = (unsigned char*)malloc(sizeof(unsigned char)*(dispNode.getImgBuf()->getBufSize()));
						if (mGYCpiedBuf != NULL)
						{
							mGYMemLock = 1;
						}
		     ...
		     if (mGYBeautyHandle > 0)
			{
				unsigned char* dispImgBuf = (unsigned char*) dispNode.getImgBuf()->getVirAddr();
				memcpy((unsigned char*)mGYCpiedBuf, (unsigned char*)dispImgBuf, sizeof(unsigned char)*(dispNode.getImgBuf()->getBufSize()));
				int result = GYBeautySkinFace(mGYCpiedBuf);
                if(result==1){
			    	memcpy((unsigned char*)dispImgBuf, (unsigned char*)mGYCpiedBuf, sizeof(unsigned char)*(dispNode.getImgBuf()->getBufSize()));
               }
			}
   	     	}
   #endif
...
updateZoom(vector<IhwScenario::PortImgInfo> &rImgIn)
{
    ...
    IhwScenario::Rect_t Src(mSensorInfo.getSensorWidth(), mSensorInfo.getSensorHeight());
...
getCfg(int32_t port, vector<IhwScenario::PortImgInfo> &rvImginfo)
{
    ...
    { 
        ...
        {
            imgInfoOut.crop = IhwScenario::Rect_t( mSensorInfo.getImgWidth(), mSensorInfo.getImgHeight(), 0, 0 );
        }
        //
...
void
PreviewCmdQueThread::
startGYBeauty(int isEnabled)
{
	MY_LOGE("gangyun tech:Preview:startGYBeauty=%d", isEnabled);
	mGYEnabledBeauty= isEnabled;
	if (mGYEnabledBeauty == 0 && mGYBeautyHandle > 0){
		//GYBeautyUnInit();
		mGYBeautyHandle = 0;
	}
}
...
void
PreviewCmdQueThread::
setGYBeautySmooth(int value)
{
	mGYBeautySmooth = value;
	if (mGYBeautyHandle > 0){
		GYBeautySetSmooth(mGYBeautySmooth);
	}
}
...
void
PreviewCmdQueThread::
setGYBeautyWhitening(int value)
{
	mGYBeautyWhitening = value;
	if (mGYBeautyHandle > 0){
		GYBeautySetWhitening(mGYBeautyWhitening);
	}
}
