...
void dump_cmd_log(void) 
{
	...
	while(idx != last_idx){
		MSG(INIT, "dbg_inf[%d].cmd = (0x%x, 0x%x) addr(%d, %d, %d)\n",
			...
			dbg_inf[idx].cmd.address[0],dbg_inf[idx].cmd.address[1],dbg_inf[idx].cmd.address[2]);
		if(idx == 0)
			idx = MAX_CMD_LOG_CNT;
		idx--;
		}
...
__attribute__((aligned(64))) static u8 local_buffer[16384 + 1280];
static u8 *temp_buffer_16_align;   // 16 byte aligned buffer, for HW issue
__attribute__((aligned(64))) static u8 temp_buffer[16384 + 1280];
...
{
	if(devinfo.vendor == VEND_SANDISK || devinfo.vendor == VEND_TOSHIBA)
	{
...
static void mtk_nand_turn_on_randomizer(struct mtd_info *mtd, struct nand_chip *chip ,kal_uint32 page)
{
...
static void mtk_nand_feature_rrtry(struct mtd_info *mtd,flashdev_info deviceinfo, u32 feature, bool defValue)
{
...
static void mtk_nand_precmd_rrtry(struct mtd_info *mtd,flashdev_info deviceinfo, u32 feature, bool defValue)
{
	//u32 feature = deviceinfo.feature_set.FeatureSet.readRetryStart+retryCount;
	if(FALSE == defValue)
		mtk_nand_reset();
...
static void mtk_nand_parapage_rrtry(struct mtd_info *mtd,flashdev_info deviceinfo, u32 retryCount, bool defValue)
{
	u32 i = 0;
...
int mtk_nand_exec_read_page(struct mtd_info *mtd, u32 u4RowAddr, u32 u4PageSize, u8 * pPageBuf, u8 * pFDMBuf)
{
	...
	if(retryCount != 0)
	{
	    ...
	    {
	    	MSG(INIT,"OK Read retry Buf:%x %x %x %x\n",pPageBuf[0],pPageBuf[1],pPageBuf[2],pPageBuf[3]);
