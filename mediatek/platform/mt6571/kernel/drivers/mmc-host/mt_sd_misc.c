...
static int simple_sd_ioctl_set_driving(struct msdc_ioctl* msdc_ctl)
{
    ...
    msdc_set_driving(host,host->hw,0); //Chnage driving strength for 3.3V IO
    msdc_set_driving(host,host->hw,1); //Chnage driving strength for 1.8V IO
#endif
#if DEBUG_MMC_IOCTL
...
static int simple_sd_ioctl_sd30_mode_switch(struct msdc_ioctl* msdc_ctl)
{
    ...
    #if 0 //Remove the 2 section since JB MR2 does not define MMC_CAP_MAX_CURRENT_XXX and MMC_CAP_SET_XPC_YYY
...
//Originally defined in mmc/card/block.c
