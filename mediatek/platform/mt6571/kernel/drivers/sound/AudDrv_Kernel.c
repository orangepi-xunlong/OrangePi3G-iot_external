...
static int AudDrv_Read_Procmem(char *buf,char **start, off_t offset, int count , int *eof, void *data)
{
    ...
    //len += sprintf(buf+len ,"AFE_MEMIF_MAXLEN  = 0x%x\n",Afe_Get_Reg(AFE_MEMIF_MAXLEN)); // MT6571 doesn't have AFE_MEMIF_MAXLEN.
...
static void AudDrv_Store_reg_AFE(void)
{
   ...
   //Suspend_reg.Suspend_AFE_MEMIF_MAXLEN              =   Afe_Get_Reg( AFE_MEMIF_MAXLEN        ); // MT6571 doesn't have AFE_MEMIF_MAXLEN.
...
static void AudDrv_Recover_reg_AFE(void)
{
    ...
    //Afe_Set_Reg( AFE_MEMIF_MAXLEN        ,Suspend_reg.Suspend_AFE_MEMIF_MAXLEN        ,MASK_ALL); // MT6571 doesn't have AFE_MEMIF_MAXLEN.
...
int AudDrv_Allocate_DL1_Buffer(struct file *fp, kal_uint32 Afe_Buf_Length)
{
    ...
    if (pblock->pucPhysBufAddr == 0)
    {
...
#endif
        //Afe_Set_Reg(AFE_MEMIF_MAXLEN, 0, 0x01); // MT6571 doesn't have AFE_MEMIF_MAXLEN.
#else
        ...
        //Afe_Set_Reg(AFE_MEMIF_MAXLEN, 1, 0x01); // MT6571 doesn't have AFE_MEMIF_MAXLEN.
#endif
    }
...
int AudDrv_Reassign_Buffer_In_SRAM(struct file *fp, unsigned long arg)
{
    ...
    switch (mem_type)
    {
        case MEM_DL1:
        {
            ...
            //Afe_Set_Reg(AFE_MEMIF_MAXLEN, 0x0, 0xf); // MT6571 doesn't have AFE_MEMIF_MAXLEN.
            break;
        }
        case MEM_DL2:
        {
            ...
            //Afe_Set_Reg(AFE_MEMIF_MAXLEN, 0x00, 0xf0); // MT6571 doesn't have AFE_MEMIF_MAXLEN.
            break;
        }
        case MEM_AWB:
        {
...
int AudDrv_Reassign_Buffer_In_EMI(struct file *fp, unsigned long arg)
{
    ...
    switch (mem_type)
    {
        case MEM_DL1:
        {
            Afe_Set_Reg(AFE_DL1_BASE , pblock->pucPhysBufAddr , 0xffffffff);
            Afe_Set_Reg(AFE_DL1_END  , pblock->pucPhysBufAddr + (AFE_Buffer_Size - 1) , 0xffffffff);
            //Afe_Set_Reg(AFE_MEMIF_MAXLEN, 0x1, 0xf); // MT6571 doesn't have AFE_MEMIF_MAXLEN.
            break;
        }
        case MEM_DL2:
        {
            Afe_Set_Reg(AFE_DL2_BASE , pblock->pucPhysBufAddr , 0xffffffff);
            Afe_Set_Reg(AFE_DL2_END  , pblock->pucPhysBufAddr + (AFE_Buffer_Size - 1) , 0xffffffff);
            //Afe_Set_Reg(AFE_MEMIF_MAXLEN, 0x10, 0xf0); // MT6571 doesn't have AFE_MEMIF_MAXLEN.
            break;
        }
        case MEM_AWB:
        {
