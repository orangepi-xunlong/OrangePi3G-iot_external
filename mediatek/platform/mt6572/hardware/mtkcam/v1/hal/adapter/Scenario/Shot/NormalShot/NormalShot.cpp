...
)
{
    ...
    #ifdef RY_GANGYUN_CAMERA_BEAUTY
    case eCmd_startGYBeauty:
		mGYEnabledBeauty = arg1;
		break;
    case eCmd_setGYBeautySmooth:
		mGYBeautySmooth = arg1;
		break;
    case eCmd_setGYBeautyWhitening:
...
{ 
   ...
   #ifdef RY_GANGYUN_CAMERA_BEAUTY
   pSingleShot->sendCommand(NSCamShot::ECamShot_CMD_START_GY_BEAUTY, mGYEnabledBeauty, 0, 0);
   pSingleShot->sendCommand(NSCamShot::ECamShot_CMD_SET_GY_BEAUTY_SMOOTH, mGYBeautySmooth, 0, 0);
   pSingleShot->sendCommand(NSCamShot::ECamShot_CMD_SET_GY_BEAUTY_WHITENING, mGYBeautyWhitening, 0, 0);
   #endif
...
handlePostViewData(MUINT8* const puBuf, MUINT32 const u4Size)
{
    ...
    #ifdef RY_GANGYUN_CAMERA_BEAUTY
    if (mGYEnabledBeauty == 1)
    {
	unsigned int m_handle = 0;
	m_handle = GYBeautyInit(0,  mShotParam.mi4PostviewWidth,  mShotParam.mi4PostviewHeight,YUV_420); 
	if (m_handle>0){
	   GYBeautySetSmooth( mGYBeautySmooth );
	   GYBeautySetWhitening( mGYBeautyWhitening );
	   unsigned char* puJpegBuf = (unsigned char*)puBuf;
