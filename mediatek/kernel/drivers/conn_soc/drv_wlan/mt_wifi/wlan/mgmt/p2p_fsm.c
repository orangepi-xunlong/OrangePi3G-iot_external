    ...
    )
{
    ...
    do {
        ...
        if (prP2pBssInfo->eCurrentOPMode != OP_MODE_ACCESS_POINT) {
            ...
            if ((prBssDesc->fgIsConnecting == TRUE) ||
                (prP2pBssInfo->eConnectionState!=PARAM_MEDIA_STATE_DISCONNECTED)) {

                DBGLOG(P2P, WARN, ("Deauth TxDone:New Connection is OnGoing and ignore Deauth TxDone Event\n"));

                prP2pFsmInfo = prAdapter->rWifiVar.prP2pFsmInfo;
                if (prP2pFsmInfo != NULL) {
                    prJoinInfo = &(prP2pFsmInfo->rJoinInfo);
                    if(EQUAL_MAC_ADDR(prJoinInfo->prTargetStaRec->aucMacAddr,prStaRec->aucMacAddr))
                        DBGLOG(P2P, WARN, ("Deauth TxDone:Skip Deauth if the same MAC="MACSTR"\n",MAC2STR(prStaRec->aucMacAddr)));
                }
                break;
            }
        }
