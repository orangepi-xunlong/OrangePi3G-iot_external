...
#endif 
/*----------------------------------------------------------------------------*/
#if 1 //add by jst for direction debug
static curr_direction = 0;
static struct mir3da_i2c_data *this_obj;
static ssize_t mir3da_direction_show(struct device_driver *ddri, char *buf)
{
    int ret;       
...
/*----------------------------------------------------------------------------*/
static ssize_t mir3da_direction_store(struct device_driver *ddri, char *buf, size_t count)
{
    signed char     tmp_direction = 0;
    int ret;    
   
    tmp_direction = simple_strtol(buf, NULL, 10);
    curr_direction = tmp_direction;

    if(ret = hwmsen_get_convert(curr_direction, &this_obj->cvt))
    {
        printk("invalid direction: %d\n", this_obj->hw->direction);
        return ret;
    }     
...
#endif 
static DRIVER_ATTR(direction,       S_IWUGO | S_IRUGO,  mir3da_direction_show,          mir3da_direction_store);
/*----------------------------------------------------------------------------*/
...
static int mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    this_obj = obj; //add by jst for direction debug
